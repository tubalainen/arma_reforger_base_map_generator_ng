...
# ---------------------------------------------------------------------------
# v1.4.7: no duplicate method definitions in EnfusionProjectGenerator
# (preventing recurrence of the silent-override bug found while fixing #85)
# ---------------------------------------------------------------------------


class TestNoDuplicateMethods:
    def test_class_has_no_duplicate_method_definitions(self):
        """Three methods (_generate_buildings_layer, _build_road_exclusion_zone,
        _building_exterior_ring) were each defined twice in the same class —
        Python silently used the second copy, so the first ~270 lines were
        unreachable dead code. Pin against recurrence."""
        import ast
        from pathlib import Path

        src_path = WEBAPP_DIR / "services" / "enfusion_project_generator.py"
        tree = ast.parse(src_path.read_text(encoding="utf-8"))
        cls = next(
            n for n in tree.body
            if isinstance(n, ast.ClassDef) and n.name == "EnfusionProjectGenerator"
        )
        method_names = [
        ...
        assert not duplicates, (
            f"Duplicate method definitions in EnfusionProjectGenerator "
            f"(Python silently uses the second copy, first is dead code): "
            f"{duplicates}"
        )


# ---------------------------------------------------------------------------
# v1.4.7: every generated C-syntax file carries a version banner (issue #99)
# ---------------------------------------------------------------------------


class TestGeneratorBanner:
    def test_layer_files_carry_version_banner(self, tmp_path):
        from config.enfusion import APP_VERSION
        gen = _make_gen()
        files = gen.generate_all(tmp_path)
        for key in ("default.layer", "managers.layer", "buildings.layer",
                    "roads.layer", "vegetation.layer", "water.layer",
                    "world.ent", "addon.gproj"):
            content = Path(files[key]).read_text(encoding="utf-8")
            assert content.startswith(
                f"// Generated by Arma Reforger Base Map Generator v{APP_VERSION}\n"
            ), f"{key} missing version banner"

    def test_meta_and_conf_files_skip_banner(self, tmp_path):
        """`.meta` and `.conf` are passed through unstamped — we haven't
        verified Enfusion's parser tolerates leading // comments there.
        If someone confirms it does, this test can flip to assert presence."""
        gen = _make_gen()
        files = gen.generate_all(tmp_path)
        for key in ("world.ent.meta", "mission.conf.meta", "mission.conf"):
            content = Path(files[key]).read_text(encoding="utf-8")
            assert not content.startswith("// Generated by"), (
                f"{key} unexpectedly carries the banner — confirm Enfusion "
                f"parses it correctly before flipping this test."
            )
