...
3. Download COG GeoTIFF assets with Basic Auth
4. Merge tiles and crop to requested bbox
...
async def fetch_stac_elevation(
    ...
    try:
        async with httpx.AsyncClient(timeout=180.0) as client:
            ...
            # 2. Download COG assets with Basic Auth
            import rasterio
            ...
            try:
                for feat in features:
                    ...
                    logger.info(
                        f"Downloading STAC tile: {feat.get('id', '?')} "
                    ...
                    if data_resp.status_code != 200:
                        if data_resp.status_code == 403:
                            ...
                            return None
                        logger.warning(
                            f"Failed to download tile {feat.get('id', '?')}: "
                    ...
                    memfile = rasterio.MemoryFile(content)
                    ds = memfile.open()
                ...
                logger.info(
                    f"Downloaded {len(tile_datasets)} tiles "
                ...
                logger.info(
                    f"STAC Höjd: merged {len(tile_datasets)} tile(s) "
