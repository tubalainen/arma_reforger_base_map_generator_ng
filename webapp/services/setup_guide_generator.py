...
- `buildings.layer` — every OSM building footprint is **already placed** as a stock Reforger prefab (`House_Village_E_1I01`, `ChurchSmall_E_01`, etc.). Buildings whose centroid landed on an asphalt road have been dropped (L12 de-conflict). Override per-category in `webapp/config/buildings.py::KNOWN_BUILDING_PREFABS`.
...
| `church` / `chapel`      | `ChurchSmall_E_01` |
...
Every path **and its resource GUID** was read off a Workbench-saved
`.layer` file in public Reforger mod source on GitHub — see the docstring at
[`webapp/config/buildings.py`](../webapp/config/buildings.py) for the sourcing
methodology and the per-entry provenance count.

> **Check your buildings actually appeared.** Releases up to and including
> v1.15.3 referenced building prefabs by the *addon* GUID instead of each
> `.et` file's own resource GUID. Workbench resolves nothing for that form,
> creates no entity, and **logs nothing at all** — so every building in every
> map generated by those versions was dropped without a single error message
> (issue #198). Terrain, surfaces and satellite imagery were unaffected, which
> is what made it easy to miss. It is fixed here, but the failure mode is
> invisible, so verify it once: open the world, select
> `buildings.layer` in the Layers panel, and confirm the entity count matches
> the number above. An empty layer with a non-zero count means the references
> did not resolve — file an issue with your Workbench version.
...
for `Building_House`): edit `webapp/config/buildings.py` and change **both**
the path in `KNOWN_BUILDING_PREFABS` and its GUID in `BUILDING_PREFAB_GUIDS`.
A path without a matching GUID entry is refused at startup rather than shipped
— changing the path alone would silently drop every building of that category,
which is exactly what issue #198 was.

Get the GUID by right-clicking the `.et` in the Workbench Resource Browser →
*Copy Resource GUID*, or by dragging the prefab into an empty world, saving,
and reading the `<Class> : "{{GUID}}<path>"` line out of the resulting
`<world>_Layers/default.layer`. GUIDs differ per file — even between variants
of one building — and are version-specific.

**To go back to footprint-marker mode** for a category: remove its entry from
`KNOWN_BUILDING_PREFABS` (and its GUID).
...
### Workbench crashes on world reopen after heightmap import (issue #120)
Symptom: you complete `Import Height Map…`, then double-click the world `.ent`
file to reopen, and Workbench crashes a few milliseconds after `WORLD: CreateEntities`.
`crash.log` shows `nvtt::CubeSurface::toGamma` with an access violation reading `0x8`
(a null-pointer dereference inside the cubemap baker).

Root cause: maps generated by **v1.5.6 or earlier** omit the
`GenericWorldEntity world { SkyPreset…; PlanetPreset {…}; CloudsRenderer…;
CloudsPreset…; LensFlaresConfig… }` block at the top of `default.layer`. The
v1.5.0 fix for issue #111 dropped that block while removing inline terrain
properties; restoring it correctly was the v1.5.7 fix for issue #122. Without
those material references, NVTT's cubemap bake at world load has no source
materials and dereferences a null `CubeSurface*` on reopen.

Check the version banner on line 1 of `Worlds/<MapName>_Layers/default.layer`.
If it reads `v1.5.6` or earlier, **regenerate the map with v1.5.7 or newer**.
There is no manual Workbench workaround — the missing material references must
be present before the first reload, and Workbench will not synthesize them.

After regenerating, follow the new Step 2.2 verbatim: leave **Resample heights**
unchecked (it's unchecked by default — do not enable it), and after Import use
**File > Save World** (Ctrl+S), *not* the Terrain Tool's Save button. The
Terrain Tool's Save writes the `.terr` file but does not persist world-level
changes, which is a separate hazard documented in Step 2.2.
...
### No sky/atmosphere
- v1.5.7+ authors the `GenericWorldEntity world { SkyPreset …; PlanetPreset {…};
  CloudsRenderer …; CloudsPreset …; LensFlaresConfig … }` block at the top of
  `default.layer`. After **File > Save World** and reopen the sky should appear.
...
- Open the Environment tab on `GenericWorldEntity world` and confirm the
  sky / planets / clouds slots are populated. Ocean material slots are
  intentionally empty — drag an ocean material in if your map has water
  (per Atlas 2 p.5).
- If you regenerated against an old build (≤ v1.5.6), see the "Workbench
  crashes on world reopen after heightmap import" entry above — those bundles
  also lack the env materials and will crash on reopen, not just render dark.
